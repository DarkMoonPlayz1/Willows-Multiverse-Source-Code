...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'lab': // Willow week bozo
				var bg:BGSprite = new BGSprite('lab', -600, -200, 0.9, 0.9);
				add(bg);
			case 'dark_lab':
				var bg:BGSprite = new BGSprite('dark_lab', -600, -200, 0.9, 0.9);
				add(bg);
			case 'alleys_thing':
				var bg:BGSprite = new BGSprite('alleys_thing', -600, -200, 0.9, 0.9);
				add(bg);
			case 'funtimebg':
				var bg:BGSprite = new BGSprite('funtimebg', -600, -200, 0.9, 0.9);
				add(bg);
		...
		timeTxt.setFormat(Paths.font("gotham.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		{
			case 'wretched wolf' | 'skeptical' | 'assassination' | 'funtime' | 'tutorial remix':
				credits = 'Composed by ZayPai - V2';
			default:
				credits = '';
			case 'kapow':
				credits = 'Original Song by Seberster - V2';
			case 'kaboom':
				credits = 'Original Song by Bbpanzu - V2';
			case 'thanatophobia':
				credits = 'Original Song from Blueballs Incident 2.0 - V2';
		}
		...
		var songWatermark = new FlxText(4, textYPos, 0,
			...
			+ " "
			+
			(curSong.toLowerCase() != '' ? (storyDifficulty == 3 ? "- ENCORE" : storyDifficulty == 2 ? "- HARD" : storyDifficulty == 1 ? "- HARD" : "- HARD") : "- HARD")
			+ "",
			16);
		// + " ", 16);
	 ...
	 */
		...
		// Removed code that makes single digits (combos obviously) have 2 and 3 digits (shit looks awful, well except for 2 digits)
